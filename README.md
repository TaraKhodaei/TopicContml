...
4. Additional dependencies:
    ...
    ```
* **`CONTML`**
    > You will need to compile a customized version of `CONTML`, we call it `CONTML2` because if you want to run bootstrap the standard
    contml may fail if two individuals have the same frequencies, the custom version allows for that, it also uses a default of 15
    characters for the individual names. We suggest that you create bin directory in your homedirectory and place the binaries there. The
    full Phylip version is here: <a html="https://phylipweb.github.io/phylip/">https://phylipweb.github.io/phylip/</a>
    ...
    > make contml
    > ```
    > If this fails you will need to see whether you can use one of the makefiles supplied using:
    > ```
    > make -f makefile... contml
    > ```
    > If that fails talk to someone to help compile a C program for your computer.
    >
...
Khodaei, M., Edwards, S. V., & Beerli, P. (2025). ”Estimating Genome-wide Phylogenies Using Probabilistic Topic
    Modeling.” Systematic Biology, syaf015. [https://doi.org/10.1093/sysbio/syaf015](https://doi.org/10.1093/sysbio/syaf015)
